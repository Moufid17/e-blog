...
export default function AccountProfileCard ({userDetails} : {userDetails: AccoutProfilType}) {
    ...
    const [userDetailsState, _] = useState(userDetails)
    useEffect(() => {
        router.refresh()
    }, [userDetailsState])
    return(
                ...
                <Box key="account_profil_card_stack_box">
                    <List key="profil_list" sx={{ alignItems: 'center', flexDirection: { xs: "column", lg: "row" } }}>
                        <ListItem key="profil_list_item_0" >
                            <ListItemContent>
                                <Avatar src={userDetailsState?.image ?? DEFAULT_AVATAR_IMAGE} />
                            </ListItemContent>
                        </ListItem>
                        <ListItem key="profil_list_item_1"  sx={{ gap: 2, alignItems: 'center', justifyContent: "center", direction: { md: "column", lg: "row" } }}>
                            <ListItemContent sx={{ textAlign: { xs: "center", lg: "left" } }}>
                                <Typography level="body-lg" fontWeight='bold' textTransform="uppercase">{userDetailsState?.name ?? DEFAULT_PSEUDO}</Typography>
                                <Typography level="body-md" noWrap>{userDetailsState?.jobName ?? DEFAULT_JOB_NAME}</Typography>
                            </ListItemContent>
                        </ListItem>
                    </List>
                </Box>
                <Box key="profil_tag">
                    <Chip key="profil_tag_chip" color="primary" variant="solid" sx={{ fontSize: { xs: "lg", lg: "md" } }}>{toUppercaseFirstChar(userDetailsState.accountPrivilege as string) ?? DEFAULT_ACCOUNT_PRIVILEGE}</Chip>
                </Box>
            </Stack>
                ...
                <Box key="profil_card_content">
                    <List key="profil_card_content_list" sx={{ display: 'flex', alignItems: "stretch", justifyContent: 'center', gap: 1 }}>
                        <ListItem key="profil_card_content_list_0" sx={{ justifyContent: "space-between", alignItems: 'center' }}>
                            ...
                            <Typography level="body-md" noWrap>{userDetailsState?.email ?? DEFAULT_EMAIL}</Typography>
                        </ListItem>
                        <Divider />
                        <ListItem key="profil_card_content_list_1" sx={{ justifyContent: "space-between", alignItems: 'center' }}>
                            ...
                            <Link href={userDetailsState.socialLink?.linkedin ?? DEFAULT_LINKEDIN}target="_blank">{"linkedin/me"}</Link>
                        </ListItem>
                        <Divider />
                        <ListItem key="profil_card_content_list_2" sx={{ justifyContent: "space-between", alignItems: 'center' }}>
                            ...
                            <Link href={userDetailsState.socialLink?.youtube ?? DEFAULT_YOUTUBE} target="_blank"><Typography level="body-md">{"youtube/me"}</Typography></Link>
                        </ListItem>
                        <Divider />
                        <ListItem key="profil_card_content_list_3" sx={{ justifyContent: "space-between", alignItems: 'center' }}>
                            ...
                            <Link href={userDetailsState.socialLink?.github ?? DEFAULT_GITHUB} target="_blank">{"github/me"}</Link>
                        </ListItem>
                        <Divider />
                        <ListItem key="profil_card_content_list_4" sx={{ justifyContent: "space-between", alignItems: 'center' }}>
                            ...
                            <Typography level="body-md" noWrap textAlign={"left"}>{userDetailsState.location ?? DEFAULT_LOCATION}</Typography>
                        </ListItem>
                    </List>
                </Box>
            </CardContent>
            <CardOverflow>
                <CardActions orientation="horizontal" sx={{ display: 'flex', justifyContent: 'space-around' }}>
                    <Box key={""} component={'span'} sx={{ display: 'flex', flexDirection: "column", alignItems: "center", justifyContent: 'center' }}>
                        <Typography level="title-lg">{userDetailsState?.stats?.likes ?? "-"}</Typography>
