...
export default function PostItem({ data }: { data: GetPostType }) {
    ...
    const HandleSetDataPostHeader = (key: string, value: string | boolean) => {
        switch (key) {
            case "title":
            ...
            {post &&
                <Stack key={post.id} alignItems="center" sx={{pt: "1.5rem"}} spacing={1}>
                    {isOwner(post?.owner?.email) ?
                        <PostEditHeader data={post} setPost={HandleSetDataPostHeader} isTitleError={isError.title} isCategoryError={isError.category}/>
                    :
